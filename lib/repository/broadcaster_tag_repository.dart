...
class BroadcasterTagRepository {
  ...
  Future<List<Map<String, dynamic>>> insertBroadcasterTag(broadcasterId,
      tagName, userId) async {
    ...
    List.generate(res.length, (i) {
      ...
      if (checkTag == tagName && !tagExists) {
        print(
            "This tag already exists for this broadcaster: $broadcasterId by this user: $userId");
        tagExists = true;
      }
    });
    if (!tagExists) {
  ...
  Future<List<Map<String, dynamic>>> selectAllBroadcastersByBroadcasterTag(
    ...
    List<Map> result = await db
        .rawQuery(
        'SELECT * FROM broadcaster_tags WHERE fk_tag_name=?', [tagName]);
